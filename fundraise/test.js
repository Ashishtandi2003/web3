...
const main = async () => {
  const provider = new ethers.providers.JsonRpcProvider(
    process.env.NEXT_PUBLIC_RPC_URL // Ensure this points to a valid RPC URL, possibly to Polygon's RPC
  );
  ...
  // Fetch donations for a specific address
  ...
  const DonationsData = AllDonations.map((e) => {
    return {
      donar: e.args.donar,
      amount: parseInt(e.args.amount), // Adjust this if you're dealing with Ether
